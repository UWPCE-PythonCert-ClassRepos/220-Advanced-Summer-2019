...
def populate_collection(connection_string, db_name, data_dir, file_path):
    """ Imports data from file_path into collection """
    thread_assignment_db = MongoClient(connection_string)[db_name]
    processed = 0
            ...
            if not header:
                ...
                try:
                    ...
                    logger.error("Error inserting record %s into table %s in MongoDB %s Error: %s",
                                 data, thread_assignment_db.name, MongoClient(connection_string), ex)
                    continue
...
def import_data(data_dir, *files):
    ...
    threads = [threading.Thread(target=populate_collection, args=("mongodb://localhost:27017", "assignment", data_dir,
                                                                  fp)) for fp in files]
